...
class AccountScreen extends React.Component {
  ...
  render() {
    ...
    let isNotThirdPartyAuth =
      this.props.userStore.user.authSource.length == 0 ? false : true; // hide change password if via google
    console.log(
      'isNotThirdPartyAuth',
      isNotThirdPartyAuth,
    ...
    return (
      <View
        ...
        <Text style={{marginTop: 36, marginBottom: 36, fontSize: 36}}>
          ACCOUNT
        </Text>
        ...
        <ScrollView showsVerticalScrollIndicator={false} horizontal={false}>
          <TouchableOpacity
            ...
            <View
              ...
              <Ionicons
                name={
                  this.state.toggleBooks ? 'ios-arrow-up' : 'ios-arrow-down'
                }
                size={20}
                color={'#000'}
              />
          ...
          {this.state.toggleBooks
            ...
            : books.map((book, i) => {
                return (
                  <View key={`book-${i}`} style={{marginBottom: 14}}>
                    <Text style={{fontSize: 14}} key={'book-' + i}>
                      {capitalizeFirstLetter(book.split('_')[0])}
                    </Text>
                    <Text>{book.split('_')[1].toUpperCase()}</Text>
                  </View>
                );
              })}
          ...
          {books.length == 0 ? (
            <View
              ...
              <TouchableOpacity
                onPress={() => {
                  this.props.navigation.navigate('AccountAddCode', {
                    handleOnAddBook: this.handleOnAddBook.bind(this),
                  });
                }}>
          ...
          {!isNotThirdPartyAuth ? (
            <View>
              ...
              <TouchableOpacity
                ...
                <Text style={{paddingVertical: 16, fontSize: 17}}>
                  Change Password
                </Text>
              </TouchableOpacity>
            </View>
          ...
          <Text style={{marginTop: 16, color: '#B5B5B5', paddingBottom: 50}}>
            Version 1.1.0
          </Text>
        </ScrollView>
      </View>
